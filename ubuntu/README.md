...
1. Make the script executable:
   ```bash
   chmod +x ubuntu_iso_builder.sh
   ```

2. Run the script:
   ```bash
   sudo ./ubuntu_iso_builder.sh -n 250 -p -b
   ```

3. Your output ISO will be saved to:
   ```text
   /mnt/e/iso/ubuntu/complete/ubuntu-node-250-postinstall-bypass-unattended.iso   # node-250-postinstall-bypass will depend on what flags you use.
   ```
...
| Flag | Description |
| --- | --- |
| `-n, --node <octet>` | Set Node ID / IP last octet (e.g. 155)"
...
sudo ./debian_iso_builder.sh

# Debian with interactive disk selection
sudo ./debian_iso_builder.sh --node 155 --interactive
